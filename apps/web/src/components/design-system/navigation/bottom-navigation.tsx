...
export function BottomNavigation({ items, className }: { items: BottomNavigationItem[]; className?: string }) {
  return (
    <nav className={clsx('fixed inset-x-0 bottom-0 z-20 border-t border-border/70 bg-surface-secondary/90 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] backdrop-blur-2xl', className)} aria-label="منوی اصلی">
          ...
          return (
            <a
              ...
              className={clsx(
                ...
                item.active ? 'bg-accent/12 text-accent shadow-sm' : 'text-text-secondary hover:bg-surface-primary hover:text-text-primary',
              )}
            >
              <Icon className={clsx('h-5 w-5 transition-transform duration-200', item.active && 'scale-110')} aria-hidden={true} />
