...
test('RssPersistenceService creates a podcast owner record for imported podcasts', async () => {
  const created: Array<{ model: string; data: unknown }> = [];
  const prisma = {
    $transaction: async (fn: (tx: any) => Promise<unknown>) => fn({
      feedSource: {
      ...
      user: {
        upsert: async ({ create, update }: { create: unknown; update: unknown }) => {
          created.push({ model: 'user', data: { create, update } });
          return { id: 'user-1', ...create };
        },
      },
      podcast: {
        findFirst: async () => null,
  ...
  await service.persistNormalizedFeed(normalizedFeed as never);

  assert.equal(created.filter((item) => item.model === 'user').length, 1);
  assert.equal(created.find((item) => item.model === 'podcast')?.data.ownerId, 'user-1');
});
